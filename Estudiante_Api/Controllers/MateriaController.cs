...
namespace Estudiante_Api.Controllers
{
    ...
    {
        public readonly IMateriaService _service;
        public MateriaController(IMateriaService service,BaseContext context, IMapper mapper) : base(context, mapper)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<ActionResult> Post(MateriaDto modelDto)
        {
            try
            {
                if (modelDto == null)
        ...
        [HttpPut("{id}")]
        public virtual async Task<IActionResult> Put(int id, [FromBody] MateriaDto modelDto)
        {
